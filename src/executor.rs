...
impl ToolExecutor {
    ...
    fn write_file_delta(&self, param: &str) -> String {
        ...
        let path = Path::new(&self.project_root).join(parts[0]);
        ...
        self.apply_delta(&path, old_content, new_content)
    }
    ...
    fn apply_delta(&self, path: &Path, old_content: &str, new_content: &str) -> String {
        ...
        if let Some(pos) = existing.find(old_content) {
            ...
            format!("Error: Could not find specified content in {}", path.display())
        }
    }
